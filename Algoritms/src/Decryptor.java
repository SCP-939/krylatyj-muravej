...
public class Decryptor {
    ...
    Decryptor(String password){
        key = Hash.hash_128(password);
    }

    DataBlock decrypt(DataBlock data) {
        DataBlock[] roundKeys = generateKeys(key);
        for (int i = 0; i < roundKeys.length / 2; i++) {

        }

        DataBlock left = new DataBlock(data, 0, data.size() / 2);
        DataBlock right = new DataBlock(data, data.size() / 2, data.size());
